...
//   if (code != cudaSuccess) 
//   {
//      fprintf(stderr,"CUDA_RUNTIME: %s %s %d\n", cudaGetErrorString(code), file, line);
//      if (abort) exit(code);
//   }
//}
inline void gpuAssert(cudaError_t code, const char *file, int line, bool abort=true)
...
CUDATreeLearner::CUDATreeLearner(const Config* config)
  ...
  use_bagging_ = false;
  Log::Info("This is the CUDA trainer!!");
}

CUDATreeLearner::~CUDATreeLearner() {
  Log::Fatal("dtor unimplemented");
  //::OpenCL if (ptr_pinned_gradients_) {
  //::OpenCL   queue_.enqueue_unmap_buffer(pinned_gradients_, ptr_pinned_gradients_);
  //::OpenCL }
  //::OpenCL if (ptr_pinned_hessians_) {
  //::OpenCL   queue_.enqueue_unmap_buffer(pinned_hessians_, ptr_pinned_hessians_);
  //::OpenCL }
  //::OpenCL if (ptr_pinned_feature_masks_) {
  //::OpenCL   queue_.enqueue_unmap_buffer(pinned_feature_masks_, ptr_pinned_feature_masks_);
  //::OpenCL }
}
...
void CUDATreeLearner::AllocateGPUMemory() {
  ...
  CUDASUCCESS_OR_FATAL(cudaFreeHost(pinned_gradients_));
  CUDASUCCESS_OR_FATAL(cudaMallocHost(&pinned_gradients_, allocated_num_data_ * sizeof(score_t)));
  //::OpenCL pinned_gradients_ = boost::compute::buffer(); // deallocate
  //::OpenCL pinned_gradients_ = boost::compute::buffer(ctx_, allocated_num_data_ * sizeof(score_t), 
  //::OpenCL                                            boost::compute::memory_object::read_write | boost::compute::memory_object::use_host_ptr, 
  //::OpenCL                                            ordered_gradients_.data());
  //::OpenCL ptr_pinned_gradients_ = queue_.enqueue_map_buffer(pinned_gradients_, boost::compute::command_queue::map_write_invalidate_region, 
  //::OpenCL                                                   0, allocated_num_data_ * sizeof(score_t));
  CUDASUCCESS_OR_FATAL(cudaFreeHost(pinned_hessians_));
  CUDASUCCESS_OR_FATAL(cudaMallocHost(&pinned_hessians_, allocated_num_data_ * sizeof(score_t)));
  //::OpenCL pinned_hessians_ = boost::compute::buffer(); // deallocate
  //::OpenCL pinned_hessians_  = boost::compute::buffer(ctx_, allocated_num_data_ * sizeof(score_t), 
  //::OpenCL                                            boost::compute::memory_object::read_write | boost::compute::memory_object::use_host_ptr, 
  //::OpenCL                                            ordered_hessians_.data());
  //::OpenCL ptr_pinned_hessians_ = queue_.enqueue_map_buffer(pinned_hessians_, boost::compute::command_queue::map_write_invalidate_region, 
  //::OpenCL                                                  0, allocated_num_data_ * sizeof(score_t));
  ...
  CUDASUCCESS_OR_FATAL(cudaFree(device_gradients_));
  CUDASUCCESS_OR_FATAL(cudaFree(device_hessians_));
  CUDASUCCESS_OR_FATAL(cudaMalloc(&device_gradients_, allocated_num_data_ * sizeof(score_t)));
  CUDASUCCESS_OR_FATAL(cudaMalloc(&device_hessians_,  allocated_num_data_ * sizeof(score_t)));
  //::OpenCL device_gradients_ = boost::compute::buffer(); // deallocate
  //::OpenCL device_gradients_ = boost::compute::buffer(ctx_, allocated_num_data_ * sizeof(score_t), 
  //::OpenCL                     boost::compute::memory_object::read_only, nullptr);
  //::OpenCL device_hessians_ = boost::compute::buffer(); // deallocate
  //::OpenCL device_hessians_  = boost::compute::buffer(ctx_, allocated_num_data_ * sizeof(score_t), 
  //::OpenCL                     boost::compute::memory_object::read_only, nullptr);
  ...
  CUDASUCCESS_OR_FATAL(cudaFree(device_feature_masks_));
  ...
  //::OpenCL                         boost::compute::memory_object::read_only, nullptr);
  //::OpenCL pinned_feature_masks_ = boost::compute::buffer(ctx_, num_dense_feature4_ * dword_features_, 
  //::OpenCL                                            boost::compute::memory_object::read_write | boost::compute::memory_object::use_host_ptr, 
...
//::OpenCL std::string CUDATreeLearner::GetBuildLog(const std::string &opts) {
//::OpenCL   boost::compute::program program = boost::compute::program::create_with_source(kernel_source_, ctx_);
//::OpenCL   try {
//::OpenCL     program.build(opts);
//::OpenCL   }
//::OpenCL   catch (boost::compute::opencl_error &e) {
//::OpenCL     auto error_code = e.error_code();
//::OpenCL     std::string log("No log available.\n");
//::OpenCL     // for other types of failure, build log might not be available; program.build_log() can crash
//::OpenCL     if (error_code == CL_INVALID_PROGRAM || error_code == CL_BUILD_PROGRAM_FAILURE) {
//::OpenCL       try {
//::OpenCL         log = program.build_log();
//::OpenCL       }
//::OpenCL       catch(...) {
//::OpenCL         // Something bad happened. Just return "No log available."
//::OpenCL       }
//::OpenCL     }
//::OpenCL     return log;
//::OpenCL   }
//::OpenCL   // build is okay, log may contain warnings
//::OpenCL   return program.build_log();
//::OpenCL }

//::OpenCL void CUDATreeLearner::BuildGPUKernels() {
//::OpenCL   Log::Info("Compiling OpenCL Kernel with %d bins...", device_bin_size_);
//::OpenCL   // destroy any old kernels
//::OpenCL   histogram_kernels_.clear();
//::OpenCL   histogram_allfeats_kernels_.clear();
//::OpenCL   histogram_fulldata_kernels_.clear();
//::OpenCL   // create OpenCL kernels for different number of workgroups per feature
//::OpenCL   histogram_kernels_.resize(kMaxLogWorkgroupsPerFeature+1);
//::OpenCL   histogram_allfeats_kernels_.resize(kMaxLogWorkgroupsPerFeature+1);
//::OpenCL   histogram_fulldata_kernels_.resize(kMaxLogWorkgroupsPerFeature+1);
//::OpenCL   // currently we don't use constant memory
//::OpenCL   int use_constants = 0;
//::OpenCL   OMP_INIT_EX();
//::OpenCL   #pragma omp parallel for schedule(guided)
//::OpenCL   for (int i = 0; i <= kMaxLogWorkgroupsPerFeature; ++i) {
//::OpenCL     OMP_LOOP_EX_BEGIN();
//::OpenCL     boost::compute::program program;
//::OpenCL     std::ostringstream opts;
//::OpenCL     // compile the GPU kernel depending if double precision is used, constant hessian is used, etc 
//::OpenCL     opts << " -D POWER_FEATURE_WORKGROUPS=" << i
//::OpenCL          << " -D USE_CONSTANT_BUF=" << use_constants << " -D USE_DP_FLOAT=" << int(config_->gpu_use_dp)
//::OpenCL          << " -D CONST_HESSIAN=" << int(is_constant_hessian_)
//::OpenCL          << " -cl-mad-enable -cl-no-signed-zeros -cl-fast-relaxed-math";
//::OpenCL     #if GPU_DEBUG >= 1
//::OpenCL     std::cout << "Building GPU kernels with options: " << opts.str() << std::endl;
//::OpenCL     #endif
//::OpenCL     // kernel with indices in an array
//::OpenCL     try {
//::OpenCL       program = boost::compute::program::build_with_source(kernel_source_, ctx_, opts.str());
//::OpenCL     }
//::OpenCL     catch (boost::compute::opencl_error &e) {
//::OpenCL       #pragma omp critical
//::OpenCL       {
//::OpenCL         std::cerr << "Build Options:" << opts.str() << std::endl;
//::OpenCL         std::cerr << "Build Log:" << std::endl << GetBuildLog(opts.str()) << std::endl;
//::OpenCL         Log::Fatal("Cannot build GPU program: %s", e.what());
//::OpenCL       }
//::OpenCL     }
//::OpenCL     histogram_kernels_[i] = program.create_kernel(kernel_name_);
//::OpenCL     
//::OpenCL     // kernel with all features enabled, with elimited branches
//::OpenCL     opts << " -D ENABLE_ALL_FEATURES=1";
//::OpenCL     try {
//::OpenCL       program = boost::compute::program::build_with_source(kernel_source_, ctx_, opts.str());
//::OpenCL     }
//::OpenCL     catch (boost::compute::opencl_error &e) {
...
//::OpenCL       }
//::OpenCL     }
//::OpenCL     histogram_allfeats_kernels_[i] = program.create_kernel(kernel_name_);
//::OpenCL 
//::OpenCL     // kernel with all data indices (for root node, and assumes that root node always uses all features)
//::OpenCL     opts << " -D IGNORE_INDICES=1";
//::OpenCL     try {
//::OpenCL       program = boost::compute::program::build_with_source(kernel_source_, ctx_, opts.str());
//::OpenCL     }
//::OpenCL     catch (boost::compute::opencl_error &e) {
...
//::OpenCL       }
//::OpenCL     }
//::OpenCL     histogram_fulldata_kernels_[i] = program.create_kernel(kernel_name_);
//::OpenCL     OMP_LOOP_EX_END();
//::OpenCL   }
//::OpenCL   OMP_THROW_EX();
//::OpenCL   Log::Info("GPU programs have been built");
//::OpenCL }

void CUDATreeLearner::SetupKernelArguments() {
  // do nothing if no features can be processed on GPU
  if (!num_dense_feature_groups_) {
    return;
  }
...
void CUDATreeLearner::InitGPU(int platform_id, int device_id) {
  ...
  #endif
  // initialize GPU
  // FIXME: here we repurpise device_id to mean the CUDA device
  CUDASUCCESS_OR_FATAL(cudaGetDevice(&dev_));
  {
    int ndevs;
    CUDASUCCESS_OR_FATAL(cudaGetDeviceCount(&ndevs));
    if (ndevs > device_id) {
      Log::Info("Using requested CUDA device %d", device_id);
      dev_ = device_id;
    }
  }
  

  //::OpenCL dev_ = boost::compute::system::default_device();
  //::OpenCL if (platform_id >= 0 && device_id >= 0) {
  //::OpenCL   const std::vector<boost::compute::platform> platforms = boost::compute::system::platforms();
  //::OpenCL   if ((int)platforms.size() > platform_id) {
  //::OpenCL     const std::vector<boost::compute::device> platform_devices = platforms[platform_id].devices();
  //::OpenCL     if ((int)platform_devices.size() > device_id) {
  //::OpenCL       Log::Info("Using requested OpenCL platform %d device %d", platform_id, device_id);
  //::OpenCL       dev_ = platform_devices[device_id];
  //::OpenCL     }   
  //::OpenCL   }   
  //::OpenCL }   
  ...
  CUDASUCCESS_OR_FATAL(cudaSetDevice(dev_));
  //::OpenCL ctx_ = boost::compute::context(dev_);
  //::OpenCL queue_ = boost::compute::command_queue(ctx_, dev_);
  //::OpenCL Log::Info("Using GPU Device: %s, Vendor: %s", dev_.name().c_str(), dev_.vendor().c_str());
  //::OpenCL BuildGPUKernels();
  AllocateGPUMemory();
  // setup GPU kernel arguments after we allocating all the buffers
  //::OpenCL SetupKernelArguments();
}
